...
const CartItems = () => {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            ...
            setLoading(true);
            try {
                const citiesResponse = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/cities`);
                ...
                const feesResponse = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/fees`);
                if (!feesResponse.ok) {
    ...
    const fetchAreas = async (cityId) => {
        try {
            const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/areas?cityId=${cityId}`);
            if (!response.ok) {
